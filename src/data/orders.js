export function normalizeOrder(raw, user = null) {
  ...
  return {
    ...
    giftWrapFee: Number(raw?.giftWrapFee ?? 0),
    discount: Number(raw?.discountAmount ?? 0),
    loyaltyCoinsUsed: Number(raw?.loyaltyCoinsUsed ?? 0),
    loyaltyDiscount: Number(raw?.loyaltyDiscountAmount ?? 0),
    voucherCode: raw?.voucherCode || null,
    giftWrap: Boolean(raw?.giftWrap),
